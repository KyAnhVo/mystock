...
type TickerDataHandler struct {
	dbQuerier    *db.DBQueryMachine
	apiRequester *http.Client
	logger       *slog.Logger
}

func NewTickerDataHandler(dbQuerier *db.DBQueryMachine, logger *slog.Logger) *TickerDataHandler {
	client := &http.Client{}
	return &TickerDataHandler{
		dbQuerier:    dbQuerier,
		apiRequester: client,
		logger:       logger,
	}
...
func (tickerDataHandler *TickerDataHandler) OverviewTicker(w http.ResponseWriter, r *http.Request) {
	...
	var response struct {
		...
		Primary_exchange *string `json:"primary_exchange"`
	}

	// Find in DB first. If not there, we find via handler.
	err := tickerDataHandler.dbQuerier.Querier.QueryRow(
		r.Context(),
		`SELECT ticker, name, cik, description, exchange FROM market_data.ticker
 			WHERE ticker = $1 `,
		ticker,
	).Scan(
		&response.Ticker,
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		tickerDataHandler.logger.Warn(
			"StockDataHandler: OverviewTicker: stock not found in DB",
			"ticker", ticker,
		)
		cfg := config.GetCfg()
		resp, err := tickerDataHandler.apiRequester.Get(
			cfg.StockApiHeader +
		...
		if err != nil {
			...
			tickerDataHandler.logger.Error(
		...
		if resp.StatusCode == 200 {
			...
			_, err := tickerDataHandler.dbQuerier.Querier.Exec(
				r.Context(),
				`INSERT INTO market_data.ticker (ticker, name, cik, description, exchange) 
					VALUES ($1, $2, $3, $4, $5)`,
				respBody.Result.Ticker,
				respBody.Result.Name,
				respBody.Result.CIK,
				respBody.Result.Description,
				respBody.Result.Exchange,
			)
			if err != nil {
				tickerDataHandler.logger.Warn("failed to store ticker in db", "error", err)
			}
...
func (tickerDataHandler *TickerDataHandler) GetTickers(w http.ResponseWriter, r *http.Request) {
	MAX_PAGE_SIZE := 100
	...
	rows, err := tickerDataHandler.dbQuerier.Querier.Query(
		r.Context(),
